...
function NextSid()
{
    $GLOBALS['PDO']->query("SELECT MAX(sid) AS next_sid FROM `:prefix_servers`");
    $sid = $GLOBALS['PDO']->single();
    return ($sid['next_sid']+1);
}
function NextAid()
{
    $GLOBALS['PDO']->query("SELECT MAX(aid) AS next_aid FROM `:prefix_admins`");
    $aid = $GLOBALS['PDO']->single();
