 ...
 * `web/install/includes/sql/struc.sql`). The on-disk column stays
 * `varchar(1)` / MySQL ENUM; this enum is the PHP-side typed wrapper
 ...
 * primitive) so phpstan-dba sees a string for the `varchar(1)` /
 * ENUM column. The case itself is for in-PHP type-safety only.
 *
