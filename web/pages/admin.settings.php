...
if (isset($_GET['log_clear']) && $_GET['log_clear'] == "true") {
    if ($userbank->HasAccess(ADMIN_OWNER)) {
        $GLOBALS['PDO']->query("TRUNCATE TABLE `:prefix_log`");
        $result = $GLOBALS['PDO']->execute();
    } else {
        Log::add("w", "Hacking Attempt", $userbank->GetProperty('user')." tried to clear the logs, but doesn't have access.");
    }
}
    ...
    if (isset($_POST['settingsGroup'])) {
        ...
        if ($_POST['settingsGroup'] == "mainsettings") {
            ...
            if (empty($errors)) {
                ...
                $GLOBALS['PDO']->query(
                    "REPLACE INTO `:prefix_settings` (value, setting) VALUES
                    (:title, 'template.title'),
                    (:logo, 'template.logo'),
                    (:password_minlength, 'config.password.minlength'),
                    (:debug, 'config.debug'),
                    (:dateformat, 'config.dateformat'),
                    (:intro_title, 'dash.intro.title'),
                    (:bansperpage, 'banlist.bansperpage'),
                    (:hideadminname, 'banlist.hideadminname'),
                    (:hideplayerips, 'banlist.hideplayerips'),
                    (:nocountryfetch, 'banlist.nocountryfetch'),
                    (:intro_text, 'dash.intro.text'),
                    (:lognopopup, 'dash.lognopopup'),
                    (:enableprotest, 'config.enableprotest'),
                    (:enablecomms, 'config.enablecomms'),
                    (:enablesubmit, 'config.enablesubmit'),
                    (:emailonlyinvolved, 'protest.emailonlyinvolved'),
                    (:customreasons, 'bans.customreasons'),
                    (:maxlife, 'auth.maxlife'),
                    (:maxlife_remember, 'auth.maxlife.remember'),
                    (:maxlife_steam, 'auth.maxlife.steam'),
                    (:defaultpage, 'config.defaultpage')"
                );

                $GLOBALS['PDO']->bindMultiple([
                    ':title' => $_POST['template_title'],
                    ':logo' => $_POST['template_logo'],
                    ':password_minlength' => $_POST['config_password_minlength'],
                    ':debug' => $debugmode,
                    ':dateformat' => $_POST['config_dateformat'],
                    ':intro_title' => $_POST['dash_intro_title'],
                    ':bansperpage' => $_POST['banlist_bansperpage'],
                    ':hideadminname' => $hideadmname,
                    ':hideplayerips' => $hideplayerips,
                    ':nocountryfetch' => $nocountryfetch,
                    ':intro_text' => $dash_intro_text,
                    ':lognopopup' => $lognopopup,
                    ':enableprotest' => $protest,
                    ':enablecomms' => $commslist,
                    ':enablesubmit' => $submit,
                    ':emailonlyinvolved' => $onlyinvolved,
                    ':customreasons' => $cureason,
                    ':maxlife' => $_POST['auth_maxlife'],
                    ':maxlife_remember' => $_POST['auth_maxlife_remember'],
                    ':maxlife_steam' => $_POST['auth_maxlife_steam'],
        ...
        if ($_POST['settingsGroup'] == "features") {
            ...
            $GLOBALS['PDO']->query(
                "REPLACE INTO `:prefix_settings` (value, setting) VALUES
                    ...
                    (:steamlogin, 'config.enablesteamlogin')"
            );
            $GLOBALS['PDO']->bindMultiple([
                ':export' => $exportpub,
                ':kickit' => $kickit,
                ':groupban' => $groupban,
                ':friendsban' => $friendsban,
                ':adminrehash' => $adminrehash,
                ':publiccomments' => $publiccomments,
                ':steamlogin' => $steamloginopt
            ]);
            $edit = $GLOBALS['PDO']->execute();
