...
{
	if ( !g_DB ) { DB_Connect(); }
	else if ( szAuth[0] != 'B' && szAuth[9] != 'L' ) {	/* Do not check bots nor check player with lan steamid. */
	...
	if ( iArgs < 1 ) {
		...
		if ( iTarget != -1 ) {
			...
			if ( GetClientAuthId( iTarget, AuthId_Steam2, szAuth, sizeof szAuth ) && szAuth[0] != 'B' && szAuth[9] != 'L' ) {
				...
				else { FormatEx( szQuery, sizeof szQuery, "SELECT created, %s_admins.user, ends, length, reason, RemoveType, type FROM %s_comms LEFT JOIN %s_admins ON %s_comms.aid = %s_admins.aid WHERE %s_comms.authid REGEXP '^STEAM_[0-9]:%s$' AND ((length > '0' AND ends > UNIX_TIMESTAMP()) OR RemoveType IS NOT NULL)", g_DatabasePrefix, g_DatabasePrefix, g_DatabasePrefix, g_DatabasePrefix, g_DatabasePrefix, g_DatabasePrefix, szAuth[8] ); }
			...
			else { ReplyToCommand( iClient, "Error: Could not retrieve %N's steam id.", iTarget ); } }
		else { ReplyToCommand( iClient, "Error: Could not find a target matching '%s'.", szTarget ); } }
...
{
	ReadConfig();
	if ( !g_DB || g_szLastError[0] ) { DB_Connect(); }
	return Plugin_Handled;
}
...
stock void ReadConfig()
{
	...
	if ( FileExists( szBuf ) ) {
		...
		if ( err != SMCError_Okay ) { LogError( smc.GetErrorString( err, szBuf, sizeof szBuf ) ? szBuf : "Unknown config parse error." ); } }
	else { LogError( "Can't find '%s'. Check it to exists and use `sb_reload` command (or change map) to reparse config.", szBuf ); }
}
...
stock SMCResult ReadConfig_KeyValue (SMCParser smc, const char[] szKey, const char[] szValue, bool key_quotes, bool value_quotes)
{
	if ( !strcmp( "DatabasePrefix", szKey, false ) ) {
		if ( !szValue[0] ) { g_DatabasePrefix = "sb"; }
		else { strcopy( g_DatabasePrefix, sizeof g_DatabasePrefix, szValue ); } }
...
stock void DB_Connect_Callback (Database db, const char[] szError, any aData)
{
	if ( !db ) { DB_HandleError( szError ); }
	else {
		g_DB = db;
		if ( g_szLastError[0] ) { g_szLastError[0] = '\0'; } }
	g_bConnecting = false;
}
...
stock void DB_OnClientAuthorized_Callback (Database db, DBResultSet results, const char[] szError, int iUserid)
{
	if ( results ) {
		...
		if ( iClient && results && results.FetchRow() ) {
			...
			if ( results.FetchRow() ) { iCommCount = results.FetchInt(0); }
		...
		if ( results ) {
			if ( results.RowCount ) {
				...
				while ( results.FetchRow() ) {
					...
					if ( !results.IsFieldNull(0) ) { FormatTime( szStartDate, sizeof szStartDate, "%Y-%m-%d", results.FetchInt(0) ); }
					if ( !results.IsFieldNull(1) ) {
						...
						if ( iLen > sizeof szBannedBy - 1 ) {
							for ( i = 2; i < 5; ++i ) { szBannedBy[sizeof szBannedBy - i] = '.'; } }
						else {
							for ( i = iLen; i < sizeof szBannedBy - 1; ++i ) { szBannedBy[i] = ' '; } } }
					...
					if ( !iLength ) {
						...
						if ( iLen < sizeof szLength - 1 ) { szLength[iLen] = ' '; } }	/* change the '\0' to a ' '. the original \0 at the end will still be there */
					if ( !results.IsFieldNull(2) ) { FormatTime( szEndDate, sizeof szEndDate, "%Y-%m-%d", results.FetchInt(2) ); }
					results.FetchString( 4, szReason, iReasonSize );	/* NOT NULL */
					iLen = results.FetchSize(4);
					if ( iLen > iReasonSize - 1 ) {
						for ( i = 2; i < 5; ++i ) { szReason[iReasonSize - i] = '.'; } }
					else {
						for ( i = iLen; i < iReasonSize - 1; ++i ) { szReason[i] = ' '; } }
					if ( !results.IsFieldNull(5) ) { results.FetchString( 5, szRemoveType, sizeof szRemoveType ); }
					if ( bBans ) {
						...
						if ( szCommType[0] == '1' ) { szCommType = "M"; }
						else if ( szCommType[0] == '2' ) { szCommType = "G"; }
						ReplyToCommand( iClient, "%s  %s  %s  %s  %s  %s  %s", szStartDate, szBannedBy, szLength, szEndDate, szCommType, szRemoveType, szReason ); } } }
			else { ReplyToCommand( iClient, "%sNo %ss found for %s.", PREFIX, bBans ? "ban" : "comm", szTargetName ); } }
		else { ReplyToCommand( iClient, "%sDB error while retrieving %ss for %s:\n%s", PREFIX, bBans ? "ban" : "comm" , szTargetName, szError ); } }
	delete hPack;
