...
# `web/includes/Telemetry/schema-1.lock.json`; the extractor parity
# test (TelemetrySchemaParityTest) gates the result so a sync that
# adds a typed slot fails the build until the matching extractor
# lands.
...
sync-telemetry-schema:
	...
	@echo "Done. Run ./sbpp.sh test to gate the new shape against the extractor parity test."
