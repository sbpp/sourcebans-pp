...
- `web/scripts/sourcebans.js` (the v1.x ~1.7k-line bulk file shipping
  ...
  Add Admin Individual servers access later moved from that
  checkbox grid onto a `data-multiselect` with page-tail
  `option[data-server-host]` hostname hydrate (search-form shape).
  Sister surface #1406 (Server Groups per-group server cards) still
  rides `server-tile-hydrate.js`.
...
| Open a simple confirm dialog (yes/no, no reason field) | `window.SBPP.confirm({ title, body?, confirmLabel?, cancelLabel?, danger? })` in `web/themes/default/js/theme.js` — returns `Promise<boolean>`. Injects a shared `<dialog data-testid="sbpp-confirm-dialog">` once; `danger: true` paints the confirm button as `btn--danger`. Do **not** fall back to `window.confirm` / `window.alert` when `SBPP` is missing (no-op). Informational feedback uses `SBPP.showToast`, not a modal. Native `dialog::backdrop` is styled in `theme.css` to match `.drawer-backdrop` (blur + darken). Regression: `web/tests/integration/NativeConfirmRegressionTest.php` + `admin-groups-delete.spec.ts`. For reason-required destructive actions use the dedicated modal row below. |
...
| Theme a single `<select>` (or multi-select) so closed/open chrome matches the panel Lucide chevron instead of the OS caret | Progressive enhancement in `web/themes/default/js/theme.js`: `select.select` (not `multiple`) → `.ssel` via `enhanceSelect`; `select[data-multiselect]` → `.msel` via `enhanceMultiselect`. Shared trigger/chevron/panel rules in `theme.css` (`.msel, .ssel` / `__trigger` / `__chevron` / `__panel`). Native `<select>` stays in the form (visually hidden) so GET submit + Playwright `selectOption` keep working. Opt out of single-select enhance with `data-native-select`. Skip `size > 1` listboxes. Optgroups render as `.ssel__group` headers. |
