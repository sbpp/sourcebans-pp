...
| Wire Add Admin Server access (groups + individual servers) as themed multi-selects with live hostname labels | `web/themes/default/page_admin_admins_add.tpl` (`#admin-add-server-groups` / `#admin-add-servers`, both `data-multiselect` + `data-placeholder`). Wire values stay `g{gid}` / `s{sid}` for `api_admins_add`. Collectors read `selectedOptions`. Hostname hydrate is the page-tail `option[data-server-host]` loop (same shape as `box_admin_admins_search.tpl`); `.msel` rebuilds on open + MutationObserver so rewritten option labels land in the panel/chips. Regression: `AddAdminServerHostHydrationTest` + `add-admin-server-host-hydration.spec.ts`. |
