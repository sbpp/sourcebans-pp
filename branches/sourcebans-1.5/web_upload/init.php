...
if(!defined('DEVELOPER_MODE') && !defined('IS_UPDATE'))
{
  if(file_exists(ROOT . '/install'))
  {
    die('Please delete the /install directory before using SourceBans');
  }
...
$GLOBALS['config'] = $GLOBALS['db']->GetAssoc('SELECT setting, value
                                               FROM   ' . DB_PREFIX . '_settings');

if($GLOBALS['config']['config.debug'] == 1 && !defined('DEVELOPER_MODE'))
{
...
{
  $offset = (!empty($GLOBALS['config']['config.timezone']) ? $GLOBALS['config']['config.timezone'] * 3600 : 0);
...
{
  die('Theme Error: <strong>' . $theme_name . '</strong> is not a valid theme. Must have a valid <strong>theme.conf.php</strong> file.');
}
if(!@is_writable(SB_THEMES_COMPILE))
{
  die('Theme Error: <strong>' . SB_THEMES_COMPILE . '</strong> MUST be writable.');
}

$theme                  = new Smarty();
$theme->allow_php_tag   = true;
$theme->caching         = false;
