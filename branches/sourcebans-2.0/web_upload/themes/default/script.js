...
function setServerInfo(res)
{
  ...
  $('host_'      + res.id).set('text', res.hostname);
}
...
window.addEvent('domready', function() {
  $$('*').each(function(el) {
    ...
    if(!el.id.indexOf('server_admins_'))
      x('ServerAdmins',  parseInt(el.id.substring(14)), setServerAdmins);
    if(!el.id.indexOf('host_'))
      x('ServerInfo',    parseInt(el.id.substring(5)),  setServerInfo);
    if(!el.id.indexOf('playerlist_'))
      x('ServerPlayers', parseInt(el.id.substring(11)), setServerPlayers);
  });
  ...
  $$('.refresh').each(function(el) {
    el.addEvent('click', function(e) {
      ...
      x('ServerInfo',    id, setServerInfo);
      x('ServerPlayers', id, setServerPlayers);
    });
  });
  $$('.select_theme').each(function(el) {
    el.addEvent('click', function(e) {
      e.stop();
      x('SelectTheme', this.get('rel'), setSelectTheme);
    });
  });
...
tinyMCE.init({
  ...
  elements : 'dashboard_text,message',
  skin_variant : 'silver',
