...
public OnRecieveBans(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	/**
	...
	new iTarget = ReadPackCell(hPack);
	new iOnConnect = ReadPackCell(hPack);
	...
	if(g_iPlayerBans[iTarget])
	{
		 ...
		 * Lets tell the client we succeeded. 
		 */
...
public MenuHandler_SelectPlayer(Handle:menu, MenuAction:action, param1, param2)
{
	...
	{
		if(param2 == MenuCancel_ExitBack && g_hTopMenu != INVALID_HANDLE && GetUserFlagBits(param1) & ADMFLAG_GENERIC)
...
{
	if(action == MenuAction_Cancel && param2 == MenuCancel_ExitBack && g_hTopMenu != INVALID_HANDLE && GetUserFlagBits(param1) & ADMFLAG_GENERIC)
		DisplayTopMenu(g_hTopMenu, param1, TopMenuPosition_LastCategory);
}
...
stock RequestBanInformation(iTarget, bool:bOnConnect, iClient = 0)
{
	 ...
	 */	
	decl String:sAuth[64], String:sEscapedAuth[128], String:sQuery[512];
	GetClientAuthString(iTarget, sAuth, sizeof(sAuth));
	SQL_EscapeString(g_hDatabase, sAuth[8], sEscapedAuth, sizeof(sEscapedAuth));
	Format(sQuery, sizeof(sQuery), "SELECT bid, authid, name, created, ends, ends - created AS length, reason, aid, RemovedBy, RemoveType FROM %s_bans WHERE authid REGEXP '^STEAM_[0-9]:%s$'", g_sDatabasePrefix, sEscapedAuth);
