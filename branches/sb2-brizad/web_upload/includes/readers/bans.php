<?php
require_once READER;
require_once READERS_DIR . 'comments.php';
require_once READERS_DIR . 'demos.php';

class BansReader extends SBReader
{
  public $hideinactive = false;
  public $limit        = 0;
  public $order        = SORT_DESC;
  public $page         = 1;
  public $search;
  public $sort         = 'time';
  public $type;
  
  public function prepare()
  {  }
  
  public function &execute()
  {
    $config  = SBConfig::getEnv('config');
    $db      = SBConfig::getEnv('db');
    $phrases = SBConfig::getEnv('phrases');
    
    $where   = 1;
    
    // Filter bans
    if(!empty($this->search) && !empty($this->type))
    {
      $search = $db->qstr($this->search);
      
      switch($this->type)
      {
        // Admin
        case 'admin':
          $where = 'ba.admin_id = ' . $search;
          break;
        // Date
        case 'date':
          $date  = explode(',', $search);
          $time  = mktime(0,  0,  0,  $date[1], $date[0], $date[2]);
          $time2 = mktime(23, 59, 59, $date[1], $date[0], $date[2]);
          $where = 'ba.time > ' . $time . ' AND ba.time < ' . $time2;
          break;
        // Ban
        case 'id':
          $where = 'ba.id = ' . $search;
          break;
        // Partial IP address
        case 'ip':
          $where = 'ba.ip LIKE "%' . $search . '%"';
          break;
        // Length
        case 'length':
          $where = 'ba.length = ' . $search;
          break;
        // Partial name
        case 'name':
          $where = 'ba.name LIKE "%' . $search . '%"';
          break;
        // No demos
        case 'nodemo':
          $where = 'ba.admin_id = ' . $search . ' AND NOT EXISTS (SELECT de.id FROM ' . SBConfig::getEnv('prefix') . '_demos AS de WHERE de.id = ba.id)';
          break;
        // Partial reason
        case 'reason':
          $where = 'ba.reason LIKE "%' . $search . '%"';
          break;
        // Server
        case 'server':
          $where = 'ba.server_id = ' . $search;
          break;
        // Partial Steam ID
        case 'steam':
          $where = 'ba.steam REGEXP "^STEAM_[0-9]:' . substr($search, 8) . '"';
          break;
        // Steam ID
        case 'steamid':
          $where = 'ba.steam REGEXP "^STEAM_[0-9]:' . substr($search, 8) . '$"';
          break;
        // Partial IP address, Steam ID, name or reason
        default:
          $where = 'ba.ip LIKE "%' . $search . '%" OR ba.steam REGEXP "^STEAM_[0-9]:' . substr($search, 8) . '" OR ba.name LIKE "%' . $search . '%" OR ba.reason LIKE "%' . $search . '%"';
      }
    }
    
    // Hide inactive bans
    if($this->hideinactive)
      $where .= ' AND (length = 0 OR time + length * 60 > UNIX_TIMESTAMP()) AND unban_admin_id IS NULL';
    
    // Fetch bans
    $ban_count = $db->GetOne('SELECT COUNT(id)
                              FROM   ' . SBConfig::getEnv('prefix') . '_bans AS ba
                              WHERE  ' . $where);
    $ban_list  = $db->GetAssoc('SELECT    ba.id, ba.type, ba.steam, ba.ip, ba.name, ba.reason, ba.length, ba.server_id, ba.admin_id, ba.admin_ip, ba.unban_admin_id, ba.unban_reason, ba.unban_time, ba.time,
                                          IFNULL(ad.name, "CONSOLE") AS admin_name, un.name AS unban_admin_name, co.code AS country_code, co.name AS country_name, se.ip AS server_ip, se.port AS server_port,
                                          mo.name AS mod_name, mo.icon AS mod_icon, 76561197960265728 + CAST(SUBSTR(ba.steam, 9, 1) AS UNSIGNED) + CAST(SUBSTR(ba.steam, 11) * 2 AS UNSIGNED) AS community_id,
                                          (SELECT COUNT(id)     FROM ' . SBConfig::getEnv('prefix') . '_bans   WHERE steam  = ba.steam OR  ip   = ba.ip) AS ban_count,
                                          (SELECT COUNT(ban_id) FROM ' . SBConfig::getEnv('prefix') . '_blocks WHERE ban_id = ba.id)                     AS block_count,
                                          (SELECT COUNT(id)     FROM ' . SBConfig::getEnv('prefix') . '_demos  WHERE ban_id = ba.id    AND type = ?)     AS demo_count
                                FROM      ' . SBConfig::getEnv('prefix') . '_bans      AS ba
                                LEFT JOIN ' . SBConfig::getEnv('prefix') . '_admins    AS ad ON ad.id = ba.admin_id
                                LEFT JOIN ' . SBConfig::getEnv('prefix') . '_admins    AS un ON un.id = ba.unban_admin_id
                                LEFT JOIN ' . SBConfig::getEnv('prefix') . '_countries AS co ON co.ip = ba.ip
                                LEFT JOIN ' . SBConfig::getEnv('prefix') . '_servers   AS se ON se.id = ba.server_id
                                LEFT JOIN ' . SBConfig::getEnv('prefix') . '_mods      AS mo ON mo.id = se.mod_id
                                WHERE     ' . $where             . '
                                ORDER BY  ' . $this->sort        . ' ' . ($this->order == SORT_DESC ? 'DESC' : 'ASC') .
                                ($this->limit ? ' LIMIT ' . ($this->page - 1) * $this->limit . ',' . $this->limit : ''),
                                array(BAN_TYPE));
    
    // Process bans
    foreach($ban_list as $id => &$ban)
    {
      // Check if ban has been either unbanned or expired
      if(!empty($ban['unban_admin_id']))
        $ban['status'] = $phrases['unbanned'];
      else if($ban['length'] && $ban['time'] + $ban['length'] * 60 < time())
        $ban['status'] = $phrases['expired'];
      
      // Fetch comments for this ban
      $comments_reader         = new CommentsReader();
      $comments_reader->ban_id = $id;
      $comments_reader->type   = BAN_TYPE;
      $ban['comments']         = $comments_reader->executeCached(ONE_DAY);
      
      // Fetch demos for this ban
      $demos_reader            = new DemosReader();
      $demos_reader->ban_id    = $id;
      $demos_reader->type      = BAN_TYPE;
      $ban['demos']            = $demos_reader->executeCached(ONE_DAY);
      
      // Format additional ban information
      $ban['length']           = ($ban['length'] ? Util::SecondsToString($ban['length'] * 60) : $phrases['permanent']);
    }
    
    list($ban_list, $ban_count) = SBPlugins::call('OnGetBans', $ban_list, $ban_count, $this->hideinactive, $this->limit, $this->page, $this->sort, $this->order, $this->search, $this->type);
    
    return array('count' => $ban_count,
                 'list'  => $ban_list);
  }
}
?>