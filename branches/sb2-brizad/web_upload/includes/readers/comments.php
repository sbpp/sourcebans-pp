...
class CommentsReader extends SBReader
{
  ...
  public function &execute()
  {
    ...
    $comments = $db->GetAssoc('SELECT    co.id, co.admin_id, co.message, co.time, co.edit_time, ad.name AS admin_name, ed.name AS edit_admin_name
                               FROM      ' . SBConfig::getEnv('prefix') . '_comments AS co
                               LEFT JOIN ' . SBConfig::getEnv('prefix') . '_admins   AS ad ON ad.id = co.admin_id
                               LEFT JOIN ' . SBConfig::getEnv('prefix') . '_admins   AS ed ON ed.id = co.edit_admin_id
                               WHERE     co.ban_id = ?
                                 AND     co.type   = ?
                               ORDER BY  time DESC',
