...
| `src/components/Footer.astro` | Override of Starlight's stock per-page footer. Re-renders Starlight's stock chrome (EditLink + LastUpdated + Pagination + the optional Built-with-Starlight kudos) and appends a small "Legacy docs" affordance — the chrome-level discovery hook for `/legacy/` (the section is intentionally excluded from the main sidebar per #1333 §5). |
