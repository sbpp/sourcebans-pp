...
| Path | What it is |
| ---- | ---------- |
| `astro.config.mjs` | Site config, sidebar tree, social links, custom CSS wiring. Also registers the `@astrojs/sitemap` integration (emits `/sitemap-index.xml`; Starlight auto-injects `<link rel="sitemap">`) — SEO plumbing so every docs page is reliably discovered + indexed. |
...
| `public/robots.txt` | Allow-all crawl directive + a `Sitemap:` pointer to `https://sbpp.github.io/sitemap-index.xml` (emitted by the `@astrojs/sitemap` integration). Files under `public/` are copied to the site root, so this serves at `/robots.txt`. Keeping the docs fully crawlable helps search engines discover and index every page. |
