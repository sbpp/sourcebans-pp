...
| Login fatals with `CannotDecodeContent` / "invalid base64" / a plain `SB_SECRET_KEY is not valid base64` page. | `SB_SECRET_KEY` must be base64 (not a UUID, hex string, or random password). Generate with `openssl rand -base64 47`, set that value as `SB_SECRET_KEY`, restart. Rotating the key logs every admin out. |
