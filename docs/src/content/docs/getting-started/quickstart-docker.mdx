...
| Login fatals with `CannotDecodeContent` / "invalid base64" / a plain `SB_SECRET_KEY must be base64 that decodes to at least 32 bytes` page (or the entrypoint dies with the same text). | `SB_SECRET_KEY` must be base64 that decodes to at least 32 bytes (not a UUID, hex string, random password, or short base64). Generate with `openssl rand -base64 47`, set that value as `SB_SECRET_KEY`, restart. Rotating the key logs every admin out. |
