...
(`SOURCEBANSPP_VERSION_MAJOR` = 2; `SB_VERSION` is stamped at compile time
from the same version tiers as the panel — release tag, then
`configs/version.json`, then `git describe`, then `dev`). That is a
