...
If you maintain a copy of the panel and want to ship your own
announcements, edit `docs/public/announcements.json` in this repo
and host the file at an `https://` URL. Point the panel at it with
`define('SB_ANNOUNCEMENTS_URL', 'https://…/announcements.json');` in
`config.php`. The default fetch URL is still
`https://sbpp.github.io/announcements.json`. This repo does not
publish a Pages sibling.
